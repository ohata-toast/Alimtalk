...
| 共通 | false     | -2016      | 受信者が1,000人を超える送信リクエストを行った場合         |
...
| 共通 | false     | -2022      | イメージを添付したが、イメージリンク(imageLink)が入力されていない場合 |
| 共通 | false     | -2023      | カカともへのメッセージ本文が400文字を超える場合(イメージ添付)         |
| 共通 | false     | -2024      | カカともへのメッセージ本文が1,000文字を超える場合        |
| 共通 | false     | -2025      | 予約日時が過去の日時の場合                 |
| 共通 | false     | -2026      | 予約日時が90日よりも先の日時の場合(最大90日まで可能)       |
...
| 共通 | false     | -3100      | テンプレートお問い合わせができない状態の場合              |
...
| 共通 | false     | -4007      | ファイルサイズが500KBを超過                       |
...
<tbody>
  ...
  <tr>
		<td>3060</td>
		<td>ユーザーに送信したが受信したかどうかは不明(Polling)</td>
	</tr>
  <tr>
	<tr>
		<td>4000</td>
	...
	<tr>
		...
		<td>9998</td>
		<td>システムに問題が発生したため担当者が確認中(現在サービス提供中ではない)</td>
	</tr>
  <tr>
		<td>9999</td>
		<td>システムに問題が発生したため担当者が確認中(システムに不明なエラー発生)</td>
	</tr>
	<tr>
		<td>E900</td>
		<td>送信キーがない場合</td>
